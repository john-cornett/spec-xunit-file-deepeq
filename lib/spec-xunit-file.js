...
function writeTest(fd, test) {
	...
	if ('failed' === test.state) {
		var err = test.err;
		var new_message = {message: err.message}
		if (err.showDiff && err.actual.length > 0 && err.expected.length > 0) {
			new_message.actual = err.actual;
			new_message.expected = err.expected;
		}
		appendLine(fd, writeTag('testcase', attrs, false, writeTag('failure', {message: escape(JSON.stringify(new_message, null, 4))}, false, cdata(err.stack))));
