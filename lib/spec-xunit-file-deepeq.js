...
function writeTest(fd, test) {
	...
	if ('failed' === test.state) {
		...
		if (err.showDiff && err.actual.length > 0 && err.expected.length > 0) {
			...
			var formatted = JSON.stringify(differences, null, "    ");
			appendLine(fd, writeTag('testcase', attrs, false, writeTag('failure', {message: escape(err.message)}, false, cdata(formatted, true))));
		} else {
