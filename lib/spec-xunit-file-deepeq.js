...
function cdata(str, escape) {
	escape = (typeof escape != 'undefined') ? escape : false;
	if (escape) {
...
function writeTest(fd, test) {
	...
	if ('failed' === test.state) {
		...
		if (err.showDiff && err.actual.length > 0 && err.expected.length > 0) {
			new_message.actual = err.actual;
			new_message.expected = err.expected;
			var str = JSON.stringify(new_message, null, 4) + "\n\n\n" + escape(err.stack);
			appendLine(fd, writeTag('testcase', attrs, false, writeTag('failure', {message: escape(err.message)}, false, cdata(str.replace(/\\n/g, "&#xD;").replace(/\\"/g,"")))));
		} else {
